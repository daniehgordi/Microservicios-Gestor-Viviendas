...
public class PropiedadServiceImplement implements PropiedadService{
	...
	@Override
	public double balance(int idPropiedad) {
		double rendimiento = rendimientoNeto(idPropiedad);
		double balanceDeuda = balanceDeudas(idPropiedad);
		double balanceImpuesto = balanceImpuestos(idPropiedad);
		double resultado = rendimiento - (balanceDeuda + balanceImpuesto);
		return resultado;
	}
	...
	public double balanceDeudas(int idPropiedad) {
		...
		List<DeudaDto> listado = Arrays.asList(template.getForObject(urlDeudas + "deudas", DeudaDto[].class));
